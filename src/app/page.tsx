...
export default function Home() {
  ...
  const [userId, setUserId] = useState<number | null>(null); // store logged-in user ID
  ...
  // Fetch impact data from API routes
  useEffect(() => {
    async function fetchImpactData() {
      if (!userId) return;

      try {
        const reportsRes = await fetch(`/api/reports?userId=${userId}`);
        const reports = await reportsRes.json();

        const rewardsRes = await fetch(`/api/rewards?userId=${userId}`);
        const rewards = await rewardsRes.json();

        const tasksRes = await fetch(`/api/reports?userId=${userId}`);
        ...
        const tokensEarned = rewards.reduce(
          (total: number, reward: any) => total + (reward.points || 0),
          0
        );
  ...
  const login = async () => {
    // Example: log in and fetch user
    const res = await fetch(`/api/user?email=test@example.com`);
